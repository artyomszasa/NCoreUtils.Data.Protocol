...
public partial class ReflectionDataUtils : IDataUtils
{
    ...
    private static bool ImplementsEnumerable(Type type, [MaybeNullWhen(false)] out Type elementType)
    {
        ...
        {
            if (itype.IsConstructedGenericType && itype.GetGenericTypeDefinition() == typeof(IEnumerable<>))
            {
    ...
    public bool IsEnumerable(Type type, [MaybeNullWhen(false)] out Type elementType)
    {
        ...
        return ImplementsEnumerable(type, out elementType);
    }
    ...
    {
        _gVisit ??= typeof(IDataTypeVisitor).GetMethod(nameof(IDataTypeVisitor.Visit)) ?? throw new InvalidOperationException("Could not get generic method definition for IDataTypeVisitor.Visit.");
        _gVisit.MakeGenericMethod(type).Invoke(visitor, default);
    }
}