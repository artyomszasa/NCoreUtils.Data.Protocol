...
namespace NCoreUtils.Data.Protocol.Linq
{
    ...
    {
        abstract class ExecuteEnumerableInvoker
        {
            static readonly ConcurrentDictionary<Type, ExecuteEnumerableInvoker> _cache = new ConcurrentDictionary<Type, ExecuteEnumerableInvoker>();

            static readonly Func<Type, ExecuteEnumerableInvoker> _create =
                type => (ExecuteEnumerableInvoker)Activator.CreateInstance(typeof(ExecuteEnumerableInvoker<>).MakeGenericType(type), true);
        ...
        {
            static readonly ConcurrentDictionary<Type, ExecuteReductionInvoker> _cache = new ConcurrentDictionary<Type, ExecuteReductionInvoker>();

            static readonly Func<Type, ExecuteReductionInvoker> _create =
                type => (ExecuteReductionInvoker)Activator.CreateInstance(typeof(ExecuteReductionInvoker<>).MakeGenericType(type), true);
        ...
        static bool TryExtractQueryableCall(Expression expression, [NotNullWhen(true)] out MethodInfo? method, [NotNullWhen(true)] out IReadOnlyList<Expression>? arguments)
        {
            ...
            {
                if (methodExpression.Method.DeclaringType.Equals(typeof(Queryable)))
                {
        ...
        // FIXME: optimize: usually used whan TSource == TResult som neither async not rebox should be necessary.
